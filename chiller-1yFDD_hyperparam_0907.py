# -*- coding: utf-8 -*-
"""
Created on Tue Sep  7 15:20:32 2021
...
def predict_ALL(NN_MODEL,data_ready):
    global RMSE_mat_all, y_true, y_pred
    model = NN_MODEL
    data_test  = data_ready[163800:327600, 0:(n_input+1)]
    testX = data_test[:,0:n_input]
    testY = data_test[:,n_input]/1000000
    testX = scalerX.transform(testX)
    y_pred  = model.predict(testX)
    ...
    RMSE_mat_all = np.sqrt(mean_squared_error(y_true,y_pred))
    RMSE_mat_all = round(RMSE_mat_all,3)
...
def validation_visual_NN(data_ready, n_epoch):
    ...
    model.summary()
    nn_hist = model.fit(trainX, trainY, epochs=n_epoch, batch_size=1000)
...
######################################################################################################################
#%% BASELINE_CASE : NO FAULT in 2nd Year
...
DATA_NAME = 'chiller_unfaulted_MAC.csv'
# Data Import
data_from_csv = pd.read_csv(DATA_NAME)
data_from_csv.set_axis(['OA_temp','inlet_flowrate','inlet_temp','outlet_temp','electric_energy'],axis='columns', inplace=True)
# Data Filter : weekdays & operation hour(07-18)
...
for i in [1]:
    train_start_day = 1
    test_start_day = 260   # number of workdays in the calendar is 260
    nn_model_HKMAC = validation_visual_NN(data_tot,250)
    
    predict_ALL(nn_model_HKMAC,data_tot)
    for j in range(i,260,5):
        test_start_day = 260 + j
        predict_NN_RMSE(nn_model_HKMAC,data_tot)
        

print(("TOTAL RMSE: {}".format(RMSE_mat_all)))


plt.figure(dpi=1000, figsize=(10,4)) 
y_true_ma = (pd.DataFrame(y_true)).rolling(630*5).mean()
y_pred_ma = (pd.DataFrame(y_pred)).rolling(630*5).mean()
plt.plot(y_true_ma, color='black', label='Dashed' )
plt.plot(y_pred_ma, color= 'red')
plt.xlabel('Time(min)')
